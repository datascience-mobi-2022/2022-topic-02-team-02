...
```{r}#
#probe mit PID gene set --> abgestürzt!
PVALUES_OVERALL_PID = list()
NES_OVERALL_PID = list()
GSEA_OVERALL_PID <- vector(mode = 'list',3)
...
for (j in 1:3) {
  ...
  names(GSEA_OVERALL_PID[[j]]) = colnames(probe_liste[[j]])
  print("Hallo")
  print("ich")
  PVALUES_OVERALL_PID[[j]] = data.frame(row.names = namen_pathways)
  for (h in 1:ncol(probe_liste[[j]])){
    a = GSEA_OVERALL_PID[[j]][[h]][,3]
    PVALUES_OVERALL_PID[[j]][1:3,h] = a
  }
  print("bin")
  PVALUES_OVERALL_PID[[j]][is.na(PVALUES_OVERALL_PID[[j]])] = 1
  colnames(PVALUES_OVERALL_PID[[j]]) = colnames(probe_liste[[j]][,1:ncol(probe_liste[[j]])])
  NES_OVERALL_PID[[j]] = data.frame(row.names = namen_pathways)
  print("froh")
  for (k in 1:ncol(probe_liste[[j]])){
    t = GSEA_OVERALL_PID[[j]][[k]][,6]
    NES_OVERALL_PID[[j]][1:3,k] = t
  }
  print("dass")
  colnames(NES_OVERALL_PID[[j]]) = colnames(probe_liste[[j]][,1:ncol(probe_liste[[j]])])
...
names(GSEA_OVERALL_PID) = tumornames
```
