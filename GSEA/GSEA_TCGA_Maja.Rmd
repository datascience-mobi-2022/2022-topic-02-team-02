...
for (j in 1:33) {
  for (i in 2:(ncol(tumorlist_z[[j]]))){
  m = c(tumorlist_z[[j]][,i])
  names(m) = c(tumorlist_z[[j]]["rowname"])
...
for (h in 1:ncol(tumorlist_z[[j]])){
  a = GSEA_OVERALL[[j]][[h]][, "padj"]
  PVALUES_OVERALL[[j]][1:50,h] = a}
PVALUES_OVERALL[[j]][is.na(PVALUES_OVERALL[[j]])] = 1
colnames(PVALUES_OVERALL[[j]]) = colnames(tumorlist_z[[j]][,2:ncol(tumorlist_z[[j]])])
NES_OVERALL[[j]] = data.frame(row.names = pathway_names)
for (k in 1:ncol(tumorlist_z[[j]])){
  t = GSEA_OVERALL[[j]][[k]][, "NES"]
  NES_OVERALL[[j]][1:50,k] = t}
colnames(NES_OVERALL[[j]]) = colnames(tumorlist_z[[j]][,2:ncol(tumorlist_z[[j]])])
