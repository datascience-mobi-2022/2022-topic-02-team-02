...
```{r example GSEA loop over 1 dataframe}
...
```{r}
#second gsea with KEGG and PID

```


```{r}
#Probe des for loops mit hallmarks
...
PVALUES_OVERALL_test = list()
NES_OVERALL_test = list()

GSEA_OVERALL_test <- vector(mode = 'list',3)
...
for (j in 1:3) {
  for (i in 1:(ncol(probe_liste[[j]]))){
    ...
    m = m[order(m), drop = FALSE]
    GSEA_OVERALL_test[[j]][[i]] = fgsea(pathways = probe_geneset,
                               stats = m,
                               scoreType = "std", 
                               minSize = 50,
                               maxSize = 500,
  ...
  names(GSEA_OVERALL_test[[j]]) = colnames(probe_liste[[j]])
  print("Hallo")
  print("ich")
  PVALUES_OVERALL_test[[j]] = data.frame(row.names = namen_pathways)
  for (h in 1:ncol(probe_liste[[j]])){
    a = GSEA_OVERALL_test[[j]][[h]][,3]
    PVALUES_OVERALL_test[[j]][1:3,h] = a
  }
  print("bin")
  PVALUES_OVERALL_test[[j]][is.na(PVALUES_OVERALL_test[[j]])] = 1
  colnames(PVALUES_OVERALL_test[[j]]) = colnames(probe_liste[[j]][,1:ncol(probe_liste[[j]])])
  NES_OVERALL_test[[j]] = data.frame(row.names = namen_pathways)
  print("froh")
  for (k in 1:ncol(probe_liste[[j]])){
    t = GSEA_OVERALL_test[[j]][[k]][,6]
    NES_OVERALL_test[[j]][1:3,k] = t
  }
  print("dass")
  colnames(NES_OVERALL_test[[j]]) = colnames(probe_liste[[j]][,1:ncol(probe_liste[[j]])])
  indexes = which(PVALUES_OVERALL_test[[j]] >= 0.05, arr.ind=TRUE)
  rows = as.vector(indexes[,1])
  cols = as.vector(indexes[,2])
  for (l in 1:length(rows)) {
    NES_OVERALL_test[[j]][rows[l], cols[l]] = 0
  }
  print("es geklappt hat")
}
names(GSEA_OVERALL_test) = tumornames

```

```{r}
#probe mit MMR gene set --> abgestürzt!
PVALUES_OVERALL_MMR = list()
NES_OVERALL_MMR = list()
GSEA_OVERALL_MMR <- vector(mode = 'list',3)
...
for (j in 1:3) {
  ...
  print("GSEA done")
  names(GSEA_OVERALL_MMR[[j]]) = colnames(probe_liste[[j]])
  PVALUES_OVERALL_MMR[[j]] = data.frame(row.names = pathway_names_MMR)
  for (h in 1:ncol(probe_liste[[j]])){
    a = GSEA_OVERALL_MMR[[j]][[h]][,3]
    PVALUES_OVERALL_MMR[[j]][1:3,h] = a
  }
  PVALUES_OVERALL_MMR[[j]][is.na(PVALUES_OVERALL_MMR[[j]])] = 1
  colnames(PVALUES_OVERALL_MMR[[j]]) = colnames(probe_liste[[j]][,1:ncol(probe_liste[[j]])])
  print("PVALUE done")
  NES_OVERALL_MMR[[j]] = data.frame(row.names = pathway_names_MMR)
  for (k in 1:ncol(probe_liste[[j]])){
    t = GSEA_OVERALL_MMR[[j]][[k]][,6]
    NES_OVERALL_MMR[[j]][1:3,k] = t
  }
  colnames(NES_OVERALL_MMR[[j]]) = colnames(probe_liste[[j]][,1:ncol(probe_liste[[j]])])
...
names(GSEA_OVERALL_MMR) = tumornames
```


```{r}
#need to plot enrichment for example patient

#need to calculate mean NES across patients within tumor type --> compare (pan-cancer)
NES_mean_list = list()
for (i in 1:33){
...
#plot enrichment for each tumor type
#heatmap 1
