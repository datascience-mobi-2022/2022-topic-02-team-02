...
sig_kirc_20 = rownames(ks_sig$PAAD[1:20,])
sig_kirc = ks_sig$PRAD[1:20,]


sig_patient1= c1_patients[[21]][which(rownames(c1_patients[[21]])%in%sig_kirc_20),]
sig_patient1_df = cbind(rownames(sig_patient1),sig_patient1)
sig_patient1_order = data.matrix(sig_patient1[match(sig_kirc_20,sig_patient1_df$`rownames(sig_patient1)`),])


sig_patient2= c2_patients[[21]][which(rownames(c2_patients[[21]])%in%sig_kirc_20),]
sig_patient2_df = cbind(rownames(sig_patient2),sig_patient2)
sig_patient2_order = data.matrix(sig_patient2[match(sig_kirc_20,sig_patient2_df$`rownames(sig_patient2)`),])


sig_patient3= c3_patients[[21]][which(rownames(c3_patients[[21]])%in%sig_kirc_20),]
sig_patient3_df = cbind(rownames(sig_patient3),sig_patient3)
sig_patient3_order = data.matrix(sig_patient3[match(sig_kirc_20,sig_patient3_df$`rownames(sig_patient3)`),])
...
Immune_type = list()
for (i in 1:length(TCGA_pw_PID)){
  Immune_type[[i]] = c("Immune infiltrated","Not immune infiltrated","Neutral")
}
...
colnames(Immune_ready) = c("Cancertype","Frequency","Immunetype")


ggplot(Immune_ready, aes(fill = Immunetype, y = Frequency, x = Cancertype))+
  geom_bar(position="fill", stat="identity", aes(group = Cancertype)) + 
  scale_fill_manual(values = c("red","grey","blue")) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
