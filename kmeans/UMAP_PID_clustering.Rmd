...
km_PID_2 = kmeans(umap_umap_PID_layout, centers= 2, nstart = 200)
PID88_km = km_PID_2$cluster
PID88_c1_names = names(PID88_km[which(PID88_km == 1)]) #patient names of cluster1
PID88_c2_names = names(PID88_km[which(PID88_km == 2)]) # patient names of cluster2
fviz_cluster(km_PID_2, data.frame(umap_umap_PID_layout),
...
PID88_c1 = NES_OVERALL_PID_cl_bound[,colnames(NES_OVERALL_PID_cl_bound) %in% PID88_c1_names] # df with patients of cluster1
PID88_c2 = NES_OVERALL_PID_cl_bound[,!colnames(NES_OVERALL_PID_cl_bound) %in% PID88_c1_names] #df with patients cluster 2
PID88_c1_mean = data.frame(apply(PID88_c1, 1,mean)) #pw activity mean for patients of cluster 1
PID88_c2_mean = data.frame (apply (PID88_c2, 1, mean)) #fpr cluster 2
PID88_c1_c2 = cbind(PID88_c2_mean, PID88_c1_mean) #combine mean cluster 1 and cluster2 
PID88_c1_c2_dif = PID88_c1_mean - PID88_c2_mean #compare the mean activity scores of pathways
#high scores means upregulated in cluster 1 

#rank the compared pw-matrix to see the most differently expressed pw
PID88_c1_c2_dif_rank = data.frame(PID88_c1_c2_dif$apply.PID88_c1..1..mean.[order(PID88_c1_c2_dif[,1],decreasing = TRUE)])
PID88_rownames = c(rownames(PID88_c1_c2_dif))
PID88_rownames_rank = c(PID88_rownames[order(PID88_c1_c2_dif[,1],decreasing = TRUE)])

PID88_c1_c2_dif_rank = as.data.frame(cbind(PID88_rownames_rank, PID88_c1_c2_dif_rank))
colnames(PID88_c1_c2_dif_rank) = c("Pathway", "diff")

  
ggplot(PID88_c1_c2_dif_rank, aes(x=reorder(Pathway,diff), y=diff)) +
  geom_bar(stat="identity")+
  coord_flip()
 
library(dplyr)

PID88_c1_c2_dif_rank %>%
  filter(PID88_c1_c2_dif_rank$diff !=0) %>%
...
PID88_c1_c2_dif_rank%>%
  filter(abs(PID88_c1_c2_dif_rank$dif)>0.05)%>%
  ggplot( aes(x=reorder(Pathway,diff), y=diff)) +
  geom_bar(stat="identity")+
  coord_flip()
...
PID88_c1_c2_FC = data.matrix(PID88_c1_mean/PID88_c2_mean)
colnames(PID88_c1_c2_FC) = "FC"
PID88_c1_c2_FC[is.nan(PID88_c1_c2_FC)]=0
...
km_PID_2 = kmeans(umap_umap_PID_layout, centers= 2, nstart = 200)
PID_km2 = km_PID_2
KIRC_PID_UMAP = as.data.frame(umap_umap_PID_layout)
...
PID_c1_c2_dif_rank%>%
  filter(abs(PID_c1_c2_dif_rank$dif)>0.05)%>%
  ggplot( aes(x=reorder(Pathway,diff), y=diff)) +
  geom_bar(stat="identity")+
  coord_flip()
...
r = rownames(KIRC_PID_cl)
r23_44 = r[23:44]
print(r23_44)
```

```{r}
Linda_PID= c("PID_ENDOTHELIN_PATHWAY",                    "PID_FAK_PATHWAY",                     "PID_FCER1_PATHWAY",                     "PID_GMCSF_PATHWAY" ,                     "PID_HIF1A_PATHWAY",                     "PID_IL1_PATHWAY" ,                    "PID_IL12_2PATHWAY" ,                     "PID_IL2_1PATHWAY"  ,                     "PID_IL2_PI3K_PATHWAY" ,                    "PID_IL2_STAT5_PATHWAY",                    "PID_IL23_PATHWAY" ,                     "PID_IL27_PATHWAY" )

saveRDS(Linda_PID, file = "Linda_PID.RDS")
