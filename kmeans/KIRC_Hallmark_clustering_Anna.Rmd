...
```{r KEGG PCA}
#run pca 
KIRC_HM_pca = prcomp(t(KIRC_HM))#center = F, scale. = F???
...
barplot(KIRC_HM_pca_eigval$variance.percent, main= "KEGG Pathways", ylab='Proportion of variance in percent', xlab = "Principal Components")
```
...
KIRC_HM_kmeans = sapply(2:5, function(k){
  kmeans(x = KIRC_HM_UMAP , centers = k, nstart = 50 )})
...
KIRC_HM_kmeans_tw = sapply(1:5, function(k){
  kmeans(x = KIRC_HM_UMAP , centers = k, nstart = 50 )$tot.withinss})
...
KIRC_HM_kmeans_c = sapply(2:7, function(k){
  kmeans(x = KIRC_HM_UMAP , centers = k, nstart = 50 )$cluster})
...
library(factoextra)
#for 2 centers
km.2 = kmeans(KIRC_HM_UMAP, centers = 2, nstart = 200)
...
# for 3 centers
km.3 = kmeans(KIRC_HM_UMAP, centers = 3, nstart = 50)
...
# for 4 centers
km.4 = kmeans(KIRC_HM_UMAP, centers = 4, nstart = 50)
...
HM_km2 =kmeans(KIRC_HM_UMAP, 2 ,nstart = 50)
...
```{r PID calculations (wilcoxon, FC)}
...
HM_c1_min = apply(HM_c1, 1, min)
HM_c1_max = apply(HM_c1, 1, max) 
HM_c1_constant = c(names(which(HM_c1_min==HM_c1_max)))
length(HM_c1_constant)#there are 271 pw with constant NES, hence we use wilcoxon test 
...
alpha_HM=0.05/(nrow(HM_wilcox_pv))#0.000255102
#how many pathways have sign different NES??
length(which(HM_wilcox_pv<alpha_HM))#10
