...
Seurat_PID = CreateSeuratObject(counts= KIRC_PID_cl)
VlnPlot(Seurat_PID, features = c("PID-TCR-PATHWAY"))

all.genes = rownames(Seurat_PID)
Seurat_PID = ScaleData(Seurat_PID, features = all.genes, verbose = FALSE)#scaling 
Seurat_PID = FindVariableFeatures(object = Seurat_PID)  #damit PCA läuft, sonst fehler
Seurat_PID = RunPCA(Seurat_PID, features = VariableFeatures(object = Seurat_PID), approx = FALSE)# approx = F, because: "Hi, this is a warning not an error, just saying that you're running partial SVD but computing most (or all) of the singular values. You can set approx=FALSE to run standard SVD instead."
...
Seurat_PID = FindNeighbors(Seurat_PID, dims =1:34)#dims verändern um später unterschieldliche zuornung in UMAP zu haben
...
cluster1.marker = FindMarkers(Seurat_PID, ident.1 = "cluster1", min.pct = 0.25)#find markers --> runs a wilcox beteen clusters to identify markers
VlnPlot(Seurat_PID, features = c(row.names(cluster1.marker)[1:nrow(cluster1.marker)]))#plot the markers


Seurat_PID_markers = FindAllMarkers(Seurat_PID, min.pct = 0.25) #fidn markers for all clusters, in our case same as before because we only compare 2 clusters
...
FeaturePlot(Seurat_PID, features = Seurat_PID_top_pw$gene[1:10]) #umap with color for pw activity
