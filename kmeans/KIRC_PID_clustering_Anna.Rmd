...
```{r PCA}
#run pca 
KIRC_PID_pca = prcomp(t(KIRC_PID))#center = F, scale. = F???
...
KIRC_PID_kmeans = sapply(2:5, function(k){
  kmeans(x = KIRC_PID_UMAP , centers = k, nstart = 50 )})
...
KIRC_PID_kmeans_tw = sapply(1:5, function(k){
  kmeans(x = KIRC_PID_UMAP , centers = k, nstart = 50 )$tot.withinss})
...
KIRC_PID_kmeans_c = sapply(2:7, function(k){
  kmeans(x = KIRC_PID_UMAP , centers = k, nstart = 50 )$cluster})
...
PID_km2 =kmeans(KIRC_PID_UMAP, 2 ,nstart = 50)
