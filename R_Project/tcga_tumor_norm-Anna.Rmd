...
#calculation of log2(foldchange = mean  cond 1/mean cond 2)
tcga_KIRC_cl_abs_logFC = data.matrix(log2(tcga_KIRC_tumor_cl_abs_Meanexpr/tcga_KIRC_norm_cl_abs_Meanexpr))
...
norm_notnd=which(tcga_KIRC_norm_cl_shapiro>0.05)
#extract the gene names that are not normal distributed (=notnd)/normal distrtibuted
tcga_KIRC_norm_cl_sh_notnd = as.matrix((tcga_KIRC_norm_cl_shapiro[norm_notnd,]))
tcga_KIRC_norm_cl_sh_nd= as.matrix((tcga_KIRC_norm_cl_shapiro[-norm_notnd,]))
nrow(tcga_KIRC_norm_cl_shapiro)==nrow(tcga_KIRC_norm_cl_sh_nd)+nrow(tcga_KIRC_norm_cl_sh_notnd)#TRUE--> it worked
#same for tumor data
tumor_notnd=which(tcga_KIRC_tumor_cl_shapiro>0.05)
tcga_KIRC_tumor_cl_sh_notnd = as.matrix((tcga_KIRC_tumor_cl_shapiro[tumor_notnd,]))
tcga_KIRC_tumor_cl_sh_nd= as.matrix((tcga_KIRC_tumor_cl_shapiro[-tumor_notnd,]))
nrow(tcga_KIRC_tumor_cl_shapiro)==nrow(tcga_KIRC_tumor_cl_nd)+nrow(tcga_KIRC_tumor_cl_notnd)
```
```{r}
...
tcga_KIRC_norm_cl_notnd_names = c(rownames(tcga_KIRC_norm_cl_sh_notnd))#rownames of not normal distributed genes in normal tissue
tcga_KIRC_tumor_cl_notnd_names = c(rownames(tcga_KIRC_tumor_cl_sh_notnd))# in tumor tissue
...
alpha=0.05/(nrow(tcga_KIRC_norm_cl))
...
#row 3791 is NaN --> no difference between norm and tumor data, problenm because paired ttest
sum(tcga_KIRC_tumor_cl_nd[3791,]-tcga_KIRC_norm_cl_nd[3791,])
```


```{r}
#Combine both matrices of pvalues (wilcox and ttest) and FC
tcga_KIRC_cl_pv = rbind(tcga_KIRC_cl_Expr_tt_pv, tcga_KIRC_cl_Expr_wilcox_pv)
#order the matrix according to gene names because otherwise no matching with FC matrix(as the pv matrix is ordered by nd and not nd genes)
tcga_KIRC_cl_pv_ord = data.matrix(tcga_KIRC_cl_pv[ order(row.names(tcga_KIRC_cl_pv)), ])
#ordering the FC matrix as well to match the values according to ordered pv matrix
tcga_KIRC_cl_abs_logFC_ord = data.matrix(tcga_KIRC_cl_abs_logFC[ order(row.names(tcga_KIRC_cl_abs_logFC)), ])
colnames(tcga_KIRC_cl_abs_logFC_ord) = "log2_FC"
#for plotting: -log10 of pvalue
tcga_KIRC_cl_pv_ord_log =-log10(tcga_KIRC_cl_pv_ord)
colnames(tcga_KIRC_cl_pv_ord_log) = "neg_log10_pv"
#combining FC and pv matrix
tcga_KIRC_cl_pv_FC =as.data.frame(cbind(tcga_KIRC_cl_abs_logFC_ord,tcga_KIRC_cl_pv_ord_log))
sum(is.na(tcga_KIRC_cl_Expr_tt_pv))
```
```{r}
...
tcga_KIRC_cl_pv_FC_sigup = tcga_KIRC_cl_pv_FC_sig[which(tcga_KIRC_cl_pv_FC_sig$log2_FC>0),]
```
...
library(ggplot2)
#plotting
ggplot(tcga_KIRC_cl_pv_FC, aes(log2_FC,neg_log10_pv, color = "non-significant genes"))+
  geom_point( size =0.3)+
  geom_point(data= tcga_KIRC_cl_pv_FC_sig, aes(log2_FC, neg_log10_pv, color = "downregulated genes"), size=0.3, color = "orange")+
  geom_point(data = tcga_KIRC_cl_pv_FC_sigup, aes(log2_FC,neg_log10_pv, color = "upregulated genes"), size=0.3, color = "blue")+
...
```{r}
hist(tcga_KIRC_cl_pv, breaks= 100); abline(v=alpha, col ="orange"); abline(v=0.05, col = "blue")
```
