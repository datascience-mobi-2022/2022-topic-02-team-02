...
```{r}
#try with convertid package
convert.bm(first_sorted, id = "rownames", biom.data.set = "human", biom.mart = "ensembl", host = "https://www.ensembl.org", biom.filter = "ensembl_gene_id", biom.attributes = "entrezgene_id", sym.col = "entrezgene_id")
#didnt work
```

```{r}
#try with annotationdb
n = mapIds(org.Hs.eg.db,
                    keys=row.names(first_sorted), #Column containing Ensembl gene ids
                    column="ENSEMBL",
                    keytype="ENTREZID",
                    multiVals="first")
#didnt work
...
```{r}
#try ChIPpeakAnno
entrezIDs = convert2EntrezID(IDs=first_ensemble_id, orgAnn="org.Hs.eg.db",
 ID_type="ensembl_gene_id")
#returns 9446 elements --> not all entrez ids coulb be found --> same problem HOW MATCH?
```


```{r}
#create GSEA compatible gene set list, NCBI entrez gene id as characters
carbohydrates = as.character(Carbohydrates$entrez_gene)
glycolysis = as.character(Glycolysis$entrez_gene)
amino_acid = as.character(Amino_Acid$entrez_gene)
sphingolipid = as.character(Sphingolipid$entrez_gene)
fatty_acid = as.character(Fatty_acid$entrez_gene)
pentose_phosphate = as.character(Pentose_phosphate$entrez_gene)
ether_lipid = as.character(Ether_lipid$entrez_gene)
glutathione = as.character(Glutathione$entrez_gene)
nitrogen = as.character(Nitrogen$entrez_gene)
oxidative_phosphorylation = as.character(Oxidative_phosphorylation$entrez_gene)
purine = as.character(Purine$entrez_gene)
pyrimidine = as.character(Pyrimidine$entrez_gene)
pyruvate = as.character(Pyruvate$entrez_gene)
steroid = as.character(Steroid$entrez_gene)
steroid_hormones = as.character(Steroid_hormones$entrez_gene)

pathways = list(carbohydrates, glycolysis, amino_acid, sphingolipid, fatty_acid, pentose_phosphate, ether_lipid, glutathione, nitrogen, oxidative_phosphorylation, purine, pyrimidine, pyruvate, steroid, steroid_hormones)

names(pathways) = c("M16864_Carbohydrates", "M16111_Glycolysis", "M39570_Amino_acid", "M14857_Sphingolipid", "M699_Fatty_Acid", "M1386_Pentose_phosphate", "M2130_Ether_lipid", "M1840_Glutathione", "M4629_Nitrogen", "M19540_Oxidative_phosphorylation", "M14314_Purine", "M5109_Pyrimidine", "M7934_Pyruvate", "M5872_Steroid", "M14933_Steroid_hormones")
```


```{r}
#tried fgsea, couldn't get vector (XXX) into fitting format
fgsea = fgsea(pathways = pathways, stats = XXX, maxSize=700)
fgsea
plotEnrichment(pathways["M16864_Carbohydrates"], XXX) + labs(title="Carbohydrates")
```
