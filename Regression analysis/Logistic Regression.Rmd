...
#keep order 
colnames(quantiseq_CD8) <- factor(colnames(quantiseq_CD8), levels = colnames(quantiseq_CD8))
#plot in one
quantiseq_CD8 %>%
  gather(sample, fraction, -cell_type) %>%
  ggplot(aes(x=sample, y=fraction, fill=cell_type)) +
...
sample1 = quantiseq_cluster1_CD8 %>% gather(sample)
sample2 = quantiseq_cluster2_CD8 %>% gather(sample)
...
PID_c2_names <- readRDS("../Data Analysis/github/2022-topic-02-team-02/data/PID_c2_names.rds")
...
sum(as.numeric(predicted.classes_2[1,])) #now 92 immune infiltrated patients
...
mean(KIRC_tcga_immune_patients_r %in% immuncluster_dendro) #81,5% und 75 treffer

#add new row according to Tims cluster, if in immune cluster --> 1, else --> 0
...
#NOCHMAL MACHEN MIT TIMS NEUEN PATHWAYS UND NEUER CLUSTER ZUORDNUNG
...
#is it still???problem: many patients all NES = 0, still in cluster 2, meaning immune infiltrated and "1"

#transpose datafram e for regression
...
chisqp_tcga = 1 - pchisq(modelchi_tcga, chidf_tcga) # 2.829e-08 test-Modell leistet signifikanten Erklärungsbeitrag
...
chisqp_tcga_2 = 1 - pchisq(modelchi_tcga_2, chidf_tcga_2) # 1.519e-08 test-Modell leistet signifikanten Erklärungsbeitrag --> besser als erstes
...
chisqp_tcga_3 = 1 - pchisq(modelchi_tcga_3, chidf_tcga_3) # 1.195e-08 test-Modell leistet signifikanten Erklärungsbeitrag --> besser als erstes und zweites
```

```{r}
#predict infiltration with second logistic regression model(3) from TCGA on small KIRC data set
#KIRC_PID_cl ist pathway activity matrix kleiner KIRC Datensatz mit aktuellen PID pathways
...
sum(as.numeric(predicted.classes[,1])) #41 patients
