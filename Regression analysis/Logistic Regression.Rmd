...
quantiseq_cluster1_plot_CD8 = quantiseq_cluster1_CD8 %>%
  gather(cluster_1, fraction, -cell_type) %>%
  ggplot(aes(x=cluster_1, y=fraction, fill=cell_type)) +
    geom_bar(stat='identity') +
    coord_flip() +
...
quantiseq_cluster2_plot_CD8 = quantiseq_cluster2_CD8 %>%
  gather(cluster_2, fraction, -cell_type) %>%
  ggplot(aes(x=cluster_2, y=fraction, fill=cell_type)) +
    geom_bar(stat='identity') +
    coord_flip() +
    scale_fill_manual(values = "Blue") + 
...
KIRC_tcga_patients = as.data.frame(str_sub(colnames(KIRC_tcga_PID), 6))
...
# 4. Test-Model, nur PID_TCR_PATHWAY und PID_IL1_PATHWAY
model4_tcga = glm(Infiltration ~ PID_TCR_PATHWAY+ PID_IL1_PATHWAY, data = KIRC_tcga_PID_sigpw_t, family = binomial()) 
summary(model4_tcga)

###Omnibus-Test
modelchi_tcga_4 = model4_tcga$null.deviance - model4_tcga$deviance
chidf_tcga_4 = model4_tcga$df.null - model4_tcga$df.residual
chisqp_tcga_4 = 1 - pchisq(modelchi_tcga_4, chidf_tcga_4) # 0 ???????

#Gütemaße
n4 = length(model4_tcga$residuals)
R2nk = 1-(model4_tcga$deviance / model4_tcga$null.deviance) # McFadden 0.507
R2cs = 1-exp((model4_tcga$deviance - model4_tcga$null.deviance)/n4) # cox snell 0.401
R2n = R2cs/(1-exp(-(model4_tcga$null.deviance/n4))) #Nagelkerke 0.63
...
#OR
OR = exp(cbind(OR = coef(model4_tcga), confint(model4_tcga)))
...
wald = Anova(model4_tcga, type="II", test="Wald")
#                 Df  Chisq Pr(>Chisq)    
#PID_TCR_PATHWAY  1 23.036  1.590e-06 ***
#PID_IL1_PATHWAY  1 36.289  1.701e-09 *

#Chisq-test for whole model
chisq = anova(model4_tcga, 
      update(model4_tcga, ~1),    # update here produces null model for comparison
      test="Chisq")
...
#1       528     264.19                          
#2       530     536.38 -2  -272.19 < 2.2e-16 ***
```

```{r test logictic regression model(4) on small KIRC dataset}
#predict infiltration with second logistic regression model(4) from TCGA on small KIRC data set
#get KIRC pathway activity matrix for PID pathways
KIRC_for_model = as.data.frame(t(KIRC_PID_cl))

probabilities4 <- model4_tcga %>% predict(KIRC_for_model, type = "response")
predicted.classes4 <- data.frame(as.numeric(ifelse(probabilities > 0.5, "1", "0")))
colnames(predicted.classes4) = "Prediction"
rownames(predicted.classes4) = colnames(KIRC_PID_cl)
sum(as.numeric(predicted.classes4[,1])) #41 predicted patients
KIRC_predict4 = rbind(KIRC_PID_cl, t(predicted.classes4), cluster_numbers)
rownames(KIRC_predict4)[rownames(KIRC_predict4) == "1"] <- "Infiltration"

#accuracy? 
mean(KIRC_predict4["Prediction",] == KIRC_predict4["Infiltration",]) #0.9305556
KIRC_predict_t4 = as.data.frame(t(KIRC_predict4))

#Plot
TCR_regression_plot4 = KIRC_predict_t4 %>%
        mutate(prob = ifelse(Infiltration == 1, 1, 0)) %>%
...
IL1_regression_plot4 = KIRC_predict_t4 %>%
        mutate(prob = ifelse(Infiltration == 1, 1, 0)) %>%
...
regression_plots_4 = list(TCR_regression_plot4, IL1_regression_plot4)
ggarrange(plotlist = regression_plots_4, ncol = 2, nrow = 2)
```

```{r}
#Confusion table and FDR-plots for model4

#create dataframe with Infiltration and prediction only
predict_vs_infiltr4 = KIRC_predict_t4[,89:90]
PI_table4 = table(predict_vs_infiltr4)
PI_tibble4 = as_tibble(PI_table4)

#confusion matrix
plot_confusion_matrix(PI_tibble4, target_col = "Infiltration", 
                      prediction_col = "Prediction",
                      counts_col = "n")

#ROC-curve
pred4 = prediction(probabilities, KIRC_predict_t4$Infiltration)
