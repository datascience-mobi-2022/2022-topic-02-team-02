...
#try out ggrepel 
library(ggrepel)
ggplot(data = as.data.frame(UMAP_exp), 
       aes(x = V1, y= V2, label = cancer_types, color = cancer_types)) +
  geom_text_repel(max.overlaps = 200) +
  geom_point() #too many overlaps -> increase max.overlaps?
...
umap_plot_33 #wie loop schreiben, dass alle Tumortypen zusammen geplottet werden?? 
# wie Daten mergen? Muss das überhaupt passieren? jetzt alle in Liste UMAP_tumorlist_33
